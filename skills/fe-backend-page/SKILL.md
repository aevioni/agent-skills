...
description: 生成 backend 项目标准页面（包含搜索、表格列表、表单弹窗提交）。当用户想要新建或重构一个包含增删改查、列表页或管理页面的 Vue 页面时使用。遇到用户描述"新建一个页面"、"增加管理功能"、"做一个列表页"、"加一个增删改查"时，必须使用此技能。
---

# backend 标准页面生成规范

生成 `backend` 项目的页面时，必须严格遵循以下规范。该项目使用 Vue 3 + TypeScript + TDesign + kr36-ui 封装组件。
...
> `KrButton` 自带间距，外层用普通 `<div>` 包裹即可，无需 Tailwind flex/gap：
> ```jsx
> cell: (h, { row }) => (
>     <div>
...
> ⚠️ **`DateRangePicker` 是错误写法，禁止使用。日期范围必须用 `DatePicker` + `range: true`。**
> ⚠️ **`InputNumber` 是错误写法，禁止使用。数字输入必须用 `Input` + `type: 'number'`。**
...
- **路由权限**：在 router meta.permissions 里引用 Perms 枚举；**若用户已提供具体权限枚举（如 `Perms.KjsBackendXxxPage`）则直接使用，未提供时统一兜底为 `Perms.PAGE_VIEW`**，不设置将导致页面不显示
- 权限 key 格式为 `kjs_backend.<模块>.<子模块>.<操作>`，如 `kjs_backend.support.contract_shunt_config.add`
- 新权限需在 `src/services/permissions.ts` 中添加对应枚举项
