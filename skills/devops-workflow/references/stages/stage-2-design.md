...
> 执行阶段 2 前读本文件。将模块分析和汇总设计自动串联为一个阶段。产出物根路径见 protocols/templates.md。
...
按照讨论文档 {讨论根目录}{域}/{需求名}/discussion.md 的影响分析，分别分析各受影响模块（同一仓库内）：
如 {讨论根目录}{域}/{需求名}/docs/ 下有参考文档，先读取作为需求背景。
{对每个受影响的模块生成一行}
- {范围标签}（工作范围: {对应目录}）→ 读代码 + 读架构文档 → 产出 {讨论根目录}{域}/{需求名}/.task/analysis/{范围标签}.md
...
每个 analyst 只读自己负责的工作范围目录，不跨范围写文件。
...
```
ralph: "读取 {讨论根目录}{域}/{需求名}/.task/analysis/ 下全部分析文档，
结合 {讨论根目录}{域}/{需求名}/discussion.md 讨论文档，
如 {讨论根目录}{域}/{需求名}/docs/ 下有参考文档，一并参照。
参照 docs/cross-module.md 校验跨模块事件链（Event/Listener）的完整性。

生成：
- {讨论根目录}{域}/{需求名}/.task/design-consensus.md —— 共识/契约层，**必含清单**（缺项视为未完成）：
...
- {讨论根目录}{域}/{需求名}/.task/dev-tasks.md（含开发任务、依赖顺序；每个任务标注范围标签 + 工作范围目录 + 复杂度 简单|复杂，复杂任务在阶段 4 编码前需出 plan）
...
完成后更新 progress.md 为 PENDING_DESIGN_REVIEW，输出设计摘要，等待用户 `/devops-workflow approve`（审核走阶段 3，见 stage-3-review.md）。

> **设计分两层**：design-consensus = 共识/契约层（小需求够用）；复杂任务的实现细节留给阶段 4 的任务级 plan/LLD。详见 stage-4.0-dev.md。
