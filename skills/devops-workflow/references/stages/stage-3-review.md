...
> 执行阶段 3 / 处理 `/devops-workflow approve` 的设计审核分支前读本文件。
...
任务拆分: {讨论根目录}{域}/{需求名}/.task/dev-tasks.md
{如有冲突} 冲突记录: {讨论根目录}{域}/{需求名}/.task/conflicts.md

主 Agent 自查结论: {逐项 ✓/✗ + 缺项说明}
未决项: {N 条，处置摘要}
复杂任务: {列出将在阶段 4 出 plan 的任务}

操作: 逐项核对后执行 /devops-workflow approve；若有缺项，先让其补充再审
```
